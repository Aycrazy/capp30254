...
def run_mods(models_to_run, clfs, grid, X_training, Y_training, X_test, Y_test, outcome_col,labels, print_plots='no', custom_params = {}): #ADD Custom PARAMS
    ...
    results_df =  pd.DataFrame(columns=('model_type','clf', 'parameters', 'auc-roc','pres_at_5', 'rec_at_5','f1__at_5','supp_at_5', 'pres_at_10', 'rec_at_10','f1__at_10','supp_at_10', 'pres_at_5', 'rec_at_5','f1__at_5','supp_at_20'))
        ...
        for p in ParameterGrid(parameter_cols):
            try:
                ...
                results_df.loc[len(results_df)] = [models_to_run[index],clf, p,
                                                       roc_auc_score(Y_test, y_pred_prob)]+[score for score in scores_at_k(y_test_sorted,y_pred_prob_sorted,5.0)]+[score for score in scores_at_k(y_test_sorted,y_pred_prob_sorted,10.0)]+[score for score in scores_at_k(y_test_sorted,y_pred_prob_sorted,20.0)]
